...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Alancoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Alancoin
    // Mac: ~/Library/Application Support/Alancoin
    // Unix: ~/.Alancoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Alancoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Alancoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty Alancoin.conf if it does not exist
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Alancoind.pid"));
